...
{
	if (_vector.m_nUsed < 12) {
		_stream << "Should be more than elements." << std::endl;
		return;
	}
	...
	for (int i = _vector.m_nUsed - 1; i > _vector.m_nUsed - 6; i--)
		summary += _vector.m_pData[i];

	for (int i = _vector.m_nUsed - 9; i > _vector.m_nUsed - 13; i--)
		multiply *= _vector.m_pData[i];

	_stream << summary << std::endl;
	_stream << multiply << std::endl;
}
