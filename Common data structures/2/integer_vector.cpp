...
int IntegerVectorGetValue(const IntegerVector & _vector, int _position)
{
	return _vector.m_pData[_position];
}

void IntegerVectorShowElementOnPosition(const IntegerVector & _vector, std::istream & _is, std::ostream & _os)
{
	int position;
	while (_is) {
		_is >> position;
		if (position < 0 || position >= _vector.m_nUsed) {
			std::cout << "WRONG POSITION. ENTER IT AGAIN." << std::endl;
			continue;
		}

		if (_is)
			_os << "VALUE ON POSTION " << position << " : " << IntegerVectorGetValue(_vector, position) << std::endl;
	};
}