...
int IntegerListAccumulate(const IntegerList & _list)
{
	...
	IntegerList::Node * currentNode = _list.m_pFirst;
	int summary = 0;

	//пока есть узлы, суммируем значение и переходим к следующему
	while (currentNode)
	{
		summary += currentNode->m_value;
...
bool IntegerListIsSorted(const IntegerList & _list)
{
	//запоминаем текущий
	IntegerList::Node * currentNode = _list.m_pFirst;
	//запоминаем следующий за текущим
	IntegerList::Node * nextNode = _list.m_pFirst->m_pNext;
	
	//пока есть узлы
	while (nextNode)
	{
		//если текущий больше следующего , значит возвращаем, что неупорядочен
		if (currentNode->m_value > nextNode->m_value) 
			return false;
...
{
	//если нет разделяющего узла
	if (!_pSplitNode) return;

	//запоминаем первый узел
	IntegerList::Node * currentNode = _sourceList.m_pFirst;
	bool switcher = false;

	//пока есть узлы
	while (currentNode)
	{
		//если дошли до узла, то переводим флаг в состояние true
		if (currentNode->m_value == _pSplitNode->m_value) switcher = true;
		//если флаг == true, то во второй список значения 
		if (switcher)
			IntegerListPushBack(_targetList2, currentNode->m_value);
		//если нет, то в первый
		else
			IntegerListPushBack(_targetList1, currentNode->m_value);
