...
void IntegerVectorCustomRead(IntegerVector & _vector, std::istream & _stream)
{
	while (true)
	{
		//вводим числа
		...
		//если число меньше нуля, то удаляем последний элемент
		if (_stream && temp < 0) {
			if (!IntegerVectorIsEmpty(_vector))
				IntegerVectorPopBack(_vector);
		}
		//если отличное больше нуля , то добавляем элемент в вектор
		else if (_stream && temp != 0)
			IntegerVectorPushBack(_vector, temp);
		//если равно 0, то выходим
		else
			break;
		//печатаем состояние последовательности

		if (IntegerVectorIsEmpty(_vector))
			std::cout << "Empty vector." << std::endl; 
		else
			IntegerVectorPrint(_vector, std::cout);
	}
}