...
void IntegerVectorReserve(IntegerVector & _v, int _nCells)
{
	...
	int * pV = new int[_nCells];

	//копируем ячейки в памяти
	for (int i = 0; i < _v.m_nUsed; i++) pV[i] = _v.m_pData[i];
	
	//удаляем старые данные 
	delete[] _v.m_pData;
	
	//переназначаем указатели
	_v.m_pData = pV;
	
	//присваиваем новый размер
	_v.m_nAllocated = _nCells;
}

void IntegerVectorShrinkToFit(IntegerVector & _v)
{
	//текущий размер вектор
	int vectorSize = _v.m_nUsed;

	//создаем новый массив
	int *pV = new int[vectorSize];
	...
	//назначаем новый размер
	_v.m_nAllocated = vectorSize;
}

int IntegerVectorRemainingCapacity(const IntegerVector & _v)
{
	//возвращаем оставшиеся ячейки
	return _v.m_nAllocated - _v.m_nUsed;
}

int IntegerVectorAllocated(const IntegerVector & _v)
{
	//возвращаем переменную, хранящую кол-во мест
	return _v.m_nAllocated;
}