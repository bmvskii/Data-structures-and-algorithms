...
void IntegerListDeleteIfLower(IntegerList & _list)
{
	int inputNumber;
	while (!IntegerListIsEmpty(_list))
	{
		std::cin >> inputNumber;
		if (inputNumber < IntegerListFront(_list))
		{
			IntegerListPopFront(_list);
		}
		IntegerListPrint(_list, std::cout);
	}
}
...
int IntegerListFront(const IntegerList & _list)
{
	return _list.m_pFirst->m_value;
}