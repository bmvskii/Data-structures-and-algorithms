...
bool HashTableTryInsertElement ( HashTable & _ht, int _bucketNr, int _key, double _value )
{
...
void HashTablePrint(const HashTable & _ht, std::ostream & _s)
{
	for (int i = 0; i < _ht.m_tableSize; i++)
		if (_ht.m_pData[i].m_status == HashTable::Element::OCCUPIED)
			_s << _ht.m_pData[i].m_key << " " << _ht.m_pData[i].m_value << std::endl;
}
