...
bool IntegerVectorEquality(const IntegerVector & _v1, const IntegerVector & _v2)
{
	if (_v1.m_nUsed != _v2.m_nUsed) return false; 
	for (int i = 0; i < _v1.m_nUsed; i++)
		if (_v1.m_pData[i] != _v2.m_pData[i])
			return false;
	return true;
}