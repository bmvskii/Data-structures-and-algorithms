...
TreeNode * TreeCreateNode ( TreeNodeLabel _label )
{
    ...
    pNode->m_label   = new char[strlen(_label)+1];
	strcpy(pNode->m_label, _label);
...
{
	return _node.m_pParent ? _node.m_pParent : nullptr;
}

TreeNode * TreeGetLeftMostChildNode(TreeNode & _node)
{
	return _node.m_pLeftMostChild ? _node.m_pLeftMostChild : nullptr;
}

TreeNode * TreeGetRightSiblingNode(TreeNode & _node)
{
	return _node.m_pRightSibling ? _node.m_pRightSibling : nullptr;
}
...
{
	return _tree.m_pRoot ? _tree.m_pRoot : nullptr;
}
