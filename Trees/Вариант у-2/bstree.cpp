...
void BSTreeDestroy ( BSTree::Node * _pNode )
{
	...
	delete[] _pNode->m_key.m_lastName;
	delete _pNode;
}
...
BSTree::Node * BSTreeCreateNode ( Student _key )
{
    BSTree::Node * pNewNode = new BSTree::Node;

    pNewNode->m_key.m_mark = _key.m_mark;

	pNewNode->m_key.m_lastName = new char[strlen(_key.m_lastName) + 1];
	strcpy(pNewNode->m_key.m_lastName, _key.m_lastName);
...
void BSTreeInsertKey ( BSTree & _tree, Student _key )
{
    ...
    while ( pCurrent )
    {
        if ( pCurrent->m_key.m_mark == _key.m_mark && 
			!strcmp(pCurrent->m_key.m_lastName, _key.m_lastName))
            return;

        else if ( _key.m_mark > pCurrent->m_key.m_mark || 
			(_key.m_mark == pCurrent->m_key.m_mark && strcmp(pCurrent->m_key.m_lastName, _key.m_lastName) > 0))
        {
            if ( pCurrent->m_pLeft )
...
BSTree::Node * BSTreeFindKeyNode ( const BSTree & _tree, Student _key )
{
    BSTree::Node * pCurrent = _tree.m_pRoot;
    while ( pCurrent )
    {
        if ( _key.m_mark == pCurrent->m_key.m_mark)
            return pCurrent;
