...
BSTree * BSTreeCreate(CompareFunction function)
{
	BSTree * pTree = new BSTree;
	pTree->m_pRoot = nullptr;
	pTree->function = function;
	return pTree;
}

int BSTWalker(BSTree::Node * _pNode, int & _size)
{
	if (!_pNode)
		return 0;

	BSTWalker(_pNode->m_pLeft,_size);
	BSTWalker(_pNode->m_pRight,_size);
...
{
	int totalSize = 0;
	BSTWalker(_tree.m_pRoot, totalSize);
	return totalSize;
}
...
void BSTreeInsertKey(BSTree & _tree, const char * _key)
{
	...
	while (pCurrent)
	{
		if (!_tree.function(pCurrent->m_key, _key))
		{
			delete[]pCurrent->m_key;
			pCurrent->m_key = new char[strlen(_key)+1];
			strcpy(pCurrent->m_key, _key);
			return;
		}
			
		else if (_tree.function(_key, pCurrent->m_key) < 0)
		{
			if (pCurrent->m_pLeft)
...
BSTree::Node * BSTreeFindKeyNode(const BSTree & _tree, const char * _key)
{
	...
	while (pCurrent)
	{
		if (!_tree.function(_key, pCurrent->m_key))
			return pCurrent;
